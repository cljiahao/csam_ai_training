...
def templating(img_path):
    trackbar = read_config("./core/json/trackbar.json")

    Col_LL = np.array(
        [int(vx) for kx, vx in trackbar["hsv"].items() if kx[0] == "L"],
        dtype=np.uint8,
    )
    Col_UL = np.array(
        [int(vy) for ky, vy in trackbar["hsv"].items() if ky[0] == "H"],
        dtype=np.uint8,
    )
...
def augmenting(
    g_path, file_names, template_dict, count_dict, dataset_fols, augment_set
):
    if sum(count_dict.values()) <= augment_set["target"] / 2:
        i = 0
        for k, v in count_dict.items():
            no_of_samples = math.ceil(augment_set["target"] / (len(count_dict) * v) - 1)
