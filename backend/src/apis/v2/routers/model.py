...
async def start_train_model(
    ...
    try:
        ...
        return {"status": "training", "model_name": file_name}
    except Exception as e:
        handle_exceptions(e)
...
def start_evaluate_model(
    item: Annotated[str, Query(description="Item Type")],
    model_name: Annotated[str, Body(description="Name of the model", embed=True)],
):
    try:
        evaluate_results = evaluate_model(item, model_name)
