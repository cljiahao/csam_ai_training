...
class BatchProcessor:
    ...
    Args:
        binary_image (np.ndarray): The threshold image.
    ...
    def __init__(
        ...
        self.batch_mask = self.apply_morphology(binary_image, batch_erode, batch_close)
        self.batch_data = self._get_batch_data()

    @staticmethod
    def apply_morphology(binary_image: np.ndarray, batch_erode: int, batch_close: int):
        """Applies morphological operations to the binary image."""
        eroded_image = cv2.erode(
            binary_image, np.ones((batch_erode, batch_erode), np.uint8)
        )
        closed_image = cv2.morphologyEx(
            eroded_image, cv2.MORPH_CLOSE, np.ones((batch_close, batch_close), np.uint8)
        )
        return closed_image
    ...
    def _get_batch_data(self):
        ...
        return self._get_batch_data_from_contour_info(contour_info_list)
