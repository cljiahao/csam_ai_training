...
class BaseRepository(Generic[T]):
    ...
    def read(
        ...
        try:
            conditions = self._build_filter(filter_condition)
    ...
    def update(
        ...
        try:
            if isinstance(filter_condition, dict) and isinstance(update_data, dict):
                ...
                # Bulk update case
                count = 0
                for i, filters in enumerate(filter_condition):
                ...
                NoResultsFound(
                    "Invalid input of filter_condition and update_data type."
                )
    ...
    def delete(
        ...
        try:
            if isinstance(filter_condition, list):
                # Bulk delete case
                count = 0
                for filters in filter_condition:
                    conditions = self._build_filter(filters)
