...
class ImageSettingsService:
    ...
    def _create_settings_dict(
        erode: int, close: int, is_batch: bool, average_length: int = 0
        ...
        if average_length:
            ...
            chip_settings.update({"crop_size": crop_size})
    ...
    def update_settings(
        self, item: str, erode: int, close: int, is_batch: bool
    ) -> ImageSettings:
        """Service layer method to update settings."""
        filter_condition = self._create_filter_conditions(item)
        update_data = self._create_settings_dict(erode, close, is_batch)
